...
void labitri (int, double *, double *, double *, double *, double *, double *, double *, double *);
void lapenta (int, double *, double *, double *);
void latri (int, double *, double *, double *);
...
/* bi-tridiagonal algorithm
 *  
 *  [ A  B ][ u ]   [ fu ]
 *  [      ][   ] = [    ]
 *  [ C  D ][ v ]   [ fv ]
 *
 *  where A, B, C, D are JxJ block tridiagonal matrices,
 *  and u, v, f1, f2 are J-vectors
 */
void labitri(int J, double *A, double *B, double *C, double *D, 
              double *fu, double *fv, double *u, double *v){
	int i, j, m, n;
	double a[J][4],b[J][4], 	c[J][4], d[J][2];
	double mu, 	beta[4], 	delt[4];
	double lamb[J][4], gamm[J][2];
	...
	for (i = 0; i < J; i++){
		...
		if(i>0){
			beta[0] 	= b[i][0] - a[i][0]*lamb[i-1][0] - a[i][1]*lamb[i-1][2];
			beta[1] 	= b[i][1] - a[i][0]*lamb[i-1][1] - a[i][1]*lamb[i-1][3];
			beta[2] 	= b[i][2] - a[i][2]*lamb[i-1][0] - a[i][3]*lamb[i-1][2];
			beta[3] 	= b[i][3] - a[i][2]*lamb[i-1][1] - a[i][3]*lamb[i-1][3];

			delt[0]	= d[i][0] - a[i][0]*gamm[i-1][0] - a[i][1]*gamm[i-1][1];
			delt[1]	= d[i][1] - a[i][2]*gamm[i-1][0] - a[i][3]*gamm[i-1][1];
		}

		
		mu = beta[0]*beta[3] - beta[1]*beta[2];

		
		lamb[i][0] = (beta[3]*c[i][0]-beta[1]*c[i][2])/mu;
		lamb[i][1] = (beta[3]*c[i][1]-beta[1]*c[i][3])/mu;
		lamb[i][2] = (beta[0]*c[i][2]-beta[2]*c[i][0])/mu;
		lamb[i][3] = (beta[0]*c[i][3]-beta[2]*c[i][1])/mu;

		gamm[i][0] = (beta[3]*delt[0] - beta[1]*delt[1])/mu;
		gamm[i][1] = (beta[0]*delt[1] - beta[2]*delt[0])/mu;

	}

	// Backward compute solution
	u[J-1] = gamm[J-1][0];
	v[J-1] = gamm[J-1][1];
	for (i = J-2; i > -1; i--){
		u[i] = gamm[i][0] - lamb[i][0]*u[i+1] - lamb[i][1]*v[i+1];
		v[i] = gamm[i][1] - lamb[i][2]*u[i+1] - lamb[i][3]*v[i+1];
	}

}

void latri(int J, double *A, double *d, double *u){
	int i;
	double a[J], b[J], c[J];
	double beta[J], gamm[J];
 ...
 *  [ A ][ u ] = [ f ]
 *
 *  where A  is a JxJ pentadiagonal matrix,
 *  and u andf are J-vectors
 */
void lapenta(int J, double *A, double *f, double *u){

	int i;
	double eps = 0.000000001;
	double gamm[J], delt[J], lamb[J];
	double a[J], b[J], c[J], d[J], e[J];
	double mu, beta;
	...
	for (i = 0; i < J; i++){
		...
		if (fabs(a[0]+a[1]+b[0]+d[J-1]+e[J-2]+e[J-1]) > eps){
			printf("Error message from lapenta function. Input matrix isn't pentadiag. \n");
			exit(EXIT_FAILURE);
		}
	}
